...
- [ ] Review `eval/schemas/summary.py` to ensure `QualityMetrics`, `MetricBucket`, and `QualityMetricsStatus` are fully defined and match actual output.
- [ ] Run a test evaluation and verify `artifacts/eval/<run_id>/summary/summary.json` contains:
...
- [ ] Review `eval/rendering.py` generation logic for the Quality Metrics section.
- [ ] Generate a report from an evaluation run and verify:
...
- [ ] Ensure that failures or changes in quality metrics (e.g., CTR dropping) do **not** cause the evaluator script to exit with a non-zero status code (hard gate).
- [ ] Confirm quality metrics are treated as "soft signals" (info/warnings only) for Stage 6.

### 4. Integration Testing
- [ ] Add or extend evaluator acceptance coverage under `tests/unit/eval_platform/` (for example in `test_evaluator.py` or a new `test_stage6_acceptance.py`) to assert end-to-end summary/report quality-metric integration.
- [ ] If adding a dedicated Stage 6 file, keep scope explicit: report section presence, canonical `by_traffic_type` buckets (`real`, `synthetic`, `combined`) when corresponding events exist, and non-gating behavior.
