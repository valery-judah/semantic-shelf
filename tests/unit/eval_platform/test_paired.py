...
def test_compute_paired_deltas_duplicates_handling():
    # If there are duplicate arms for the same key, last one wins (based on implementation: pairs[r.paired_key][r.arm] = r)
    reqs = [
        create_record("1", 10.0, True, arm="baseline", paired_key="key1"),
        create_record("1", 20.0, True, arm="baseline", paired_key="key1"), # Overwrites previous baseline
