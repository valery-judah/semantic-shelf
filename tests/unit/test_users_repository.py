...
def test_create_and_get_by_external_id() -> None:
...
def test_update_preferences_top_level_merge_semantics() -> None:
    repo = UsersRepository()
    created = repo.create(
        id="usr_2",
...
def test_update_preferences_returns_unchanged_on_empty_patch() -> None:
    repo = UsersRepository()
    created = repo.create(
        id="usr_3",
...
def test_update_preferences_returns_none_for_unknown_user() -> None:
    repo = UsersRepository()
