...
def test_ingest_telemetry_events_success(client: TestClient, test_data: DataFactory):
    payload = {
        "events": [
            {
    ...
    rows = test_data.get_telemetry_events()
    assert len(rows) == 2


def test_ingest_telemetry_events_duplicate_idempotency(client: TestClient, test_data: DataFactory):
    payload = {
        "events": [
            {
                "event_name": "similar_impression",
