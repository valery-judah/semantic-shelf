...
def test_stage4_determinism_outputs(tmp_path: Path):
    """
    Test that running evaluator on the same inputs yields byte-stable outputs
    for Stage 4 components.
    """
    ...
    summary.slices = [
        SliceMetrics(
            slice_id="pop_head", sample_size=2, counts=summary.counts, latency=summary.latency
        )
    ]
    ...
    deltas_content = {
        ...
        "stats": {"count": 1, "avg_latency_delta_ms": 10.0},
    }
